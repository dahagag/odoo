...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Source Serif 4 for all
  display + Karla body + IBM Plex Mono for the index/code voice on a warm-navy code surface,
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "Karla", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "Karla", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "Karla", cqw: 2.3, weight: 500, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "Karla", cqw: 1.46, weight: 500, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "Karla", cqw: 1.35, weight: 500, tracking: "0.18em", upper: true }
  ...
  # — display ramp (Source Serif 4 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "Source Serif 4", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "Source Serif 4", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "Source Serif 4", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "Source Serif 4", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "Source Serif 4", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "Source Serif 4", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Source Serif 4 figure paired with a IBM Plex Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Source Serif 4 italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Source Serif 4 italic, small Source Serif 4 uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **Source Serif 4** (display) / **Karla** (body); ignore any preset font name lingering in prose.
...
Three editorial voices, each in its own face: **Source Serif 4** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **Karla** carries body, leads, card titles, buttons, and
...
- **Source Serif 4** (sentence case, negative-tracked) for all display; **Karla** body/chrome; **IBM Plex Mono** index + code.
...
- **Squint** — one Source Serif 4 display moment dominates at 3–6× its neighbor.
...
- **Type** — Source Serif 4 sentence-case display (negative-tracked); Karla 400 body; IBM Plex Mono kickers (uppercase 0.16em, coral ✱) + code.
...
**The ink text ladder.** `ink` / `ink-700` / `ink-500` are the teach page's three text
steps (primary / secondary / tertiary-and-labels) from `docs/teach/DESIGN-TOKENS.md`. The
preset had no keys for the lower two, so they are declared here rather than improvised
per frame. An equivalent low-alpha tint of `ink` on the cream ground is acceptable where a
frame already uses one — the two are within a hair of each other on cream — but new work
should reference these tokens.

**Fixed semantic accents (from `docs/teach/DESIGN-TOKENS.md`, NOT remixable brand hues).** The
teach page this video is embedded in carries three semantic hues the preset has no key for, and the
video must read as part of that page: **block `#A23B3B`** on **`#F3DBDB`** — refusal / hard blocker,
the one beat that must land; **teal `#2E6B60`** on **`#D9E8E3`** — met / warn-level / "Sales"; **violet
`#5B5285`** on **`#E5E2F0`** — "R&D" / technical aside. They obey the coral ration: a semantic accent
is the frame's ONE accent moment when it appears, never a second one alongside coral. Never use them
decoratively — only where they carry that exact meaning.
...
Two ramps. The **reading/chrome ramp** (Karla `body` 1.5cqw / `lead` 2.08cqw weight 400; IBM Plex
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Source Serif 4 `headline` 4.6cqw
...
- **Source Serif 4 display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Karla body** sentence case weight 400. **IBM Plex Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Source Serif 4 figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Source Serif 4 italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Source Serif 4 · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Source Serif 4 `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Source Serif 4 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Source Serif 4 line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Source Serif 4 `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a Source Serif 4 `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: Source Serif 4 italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a Source Serif 4 **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small Source Serif 4 uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** Source Serif 4 italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short Source Serif 4 sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **Source Serif 4, sentence case**, negative-tracked; **Source Serif 4 400** body; **IBM Plex Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Source Serif 4 figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Source Serif 4 display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Source Serif 4 display moment dominates.
...
- **Type** — Source Serif 4 sentence-case display (negative-tracked); Source Serif 4 400 body; IBM Plex Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
