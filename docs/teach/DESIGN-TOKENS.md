...
So today, inside the Artifact, the two flowcharts do **not** visually match the surrounding design system at all — there's no existing color-token mapping to copy. Reproducing Mermaid diagrams for `docs-build:doc` isn't a matter of finding and porting the artifact's diagram styling; that styling doesn't participate in this design system and would need to be invented from scratch (e.g. mapping `surface`/`text`/`line`/`border` to this doc's own tokens) if diagrams are ever in scope. The *mechanism* question this section originally raised (bundle Mermaid inline vs. pre-render to static SVG vs. drop diagram support) was blocked on a zero-network-requests constraint that [ADR 0009](adr/0009-docs-build-doc-relaxes-zero-network-and-adds-authoring-directives.md) has since reversed — loading Mermaid from a CDN, the same way the Claude Artifacts platform's own runtime does, is now the simplest option. The color-token mapping for rendered diagrams is still an open implementation decision — resolve it against a real ticket, not here.
