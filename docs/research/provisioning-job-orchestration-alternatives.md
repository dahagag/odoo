...
These two caveats compose: if the worker receives a batch of several same-group messages at once
(the first caveat) and then only renews visibility for the one it's actively processing, every
*other* buffered message in that batch can still expire and become available to a second worker —
the mutex leaks through the very batching SQS itself documents. Closing this requires either
requesting at most one message per `ReceiveMessage` call for these queues, or renewing visibility
for every buffered same-group receipt handle, not only the one currently being worked.
Source: [Amazon SQS Developer Guide — FIFO queue delivery logic](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/FIFO-queues-understanding-logic.html).
