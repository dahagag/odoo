...
# repository scoping — matching this repo's own `repository` claim and no other repo's (issue
# #259: ci.yml's build-image/build-prod-image jobs only ever run on `pull_request`, whose `sub`
# carries no branch component to scope on, unlike deploy_role_branch_isolation.tftest.hcl's
# push-shaped roles — and is not the plain `repo:<owner>/<repo>:pull_request` shape either once
# ID-qualified claims are enabled, which is why this condition uses the separate, plain-name
# `repository` claim instead of `sub`) — and (2) the permission policy's repository scoping —
# ecr:* is granted only against the four configured repository ARNs, with
# ecr:GetAuthorizationToken carved out into its own resource="*" statement since ECR does not
# support resource-level permissions for that action.
#
...
run "verify_ecr_push_repo_scoping" {
  ...
  assert {
    condition = anytrue([
      ...
      && toset(flatten([statement.Condition.StringEquals["token.actions.githubusercontent.com:repository"]])) == toset([
        "dahagag/odoo",
      ])
    ])
    error_message = "ecr_push's trust policy must allow sts:AssumeRoleWithWebIdentity from the GitHub OIDC provider, scoped to exactly this repo's repository claim — no more, no fewer."
  }

  # --- isolation proof: a different repository's claim does not match ---

  assert {
    condition = !contains(
      flatten([
        [for statement in jsondecode(data.aws_iam_policy_document.ecr_push_trust.json).Statement : statement][0]
        .Condition.StringEquals["token.actions.githubusercontent.com:repository"]
      ]),
      "someone-else/odoo"
    )
    error_message = "ecr_push's trust policy repository condition must not match a different repository's OIDC token."
  }
